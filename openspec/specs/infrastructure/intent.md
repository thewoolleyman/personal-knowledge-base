...
- **CI/CD** — Single `ci-cd.yml` GitHub Actions pipeline: lint (golangci-lint, gitleaks, go vet, actionlint), unit tests, acceptance tests, live API tests, e2e tests, deployment.
...
### CI pipeline
Every push to main triggers the pipeline: lint → unit tests → acceptance tests → live API tests → e2e tests → deploy. CI never silently skips tests.
